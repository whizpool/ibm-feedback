...
import SidePanelViewFeedback from "./SidePanelViewFeedback";
...
class FeedbackRatingView extends React.Component {
  ...
  componentDidMount() {
		document.title = process.env.REACT_APP_SITE_TITLE + " Feedback ";
		this.getFeedbacks();
	} 

	updateSidePanelState = (sidePanel,recordid) => {		
	...
	formateDateString = (dateStr) => {
		...
		return (<>{ (new Intl.DateTimeFormat("en-US").format(today))}<br/>{ (new Intl.DateTimeFormat('en-US', options).format(today))}</>)
	...
	viewRating = (rowIndex, recordID) => {
  	...
  	handleOnHeaderChange  = (e) => {	
		this.setState((state) => {
			const headers = columnsV2.filter((head) => {
  ...
  getFeedbacks = () => {
		...
		var config = {
			...
			data:{filters:filterData,type:'rating'}
		};		
		axios(config)
		.then(response => {
  	...
  	render() {	
		...
		var onPageChange = pageChange => {
 		...
 		return (
				...
				<SidePanelViewFeedback opensidepanel ={this.state.openSidePanel} updatesidepanelstate={this.updateSidePanelState} access_token={this.props.access_token} recordid={this.state.recordID} widgettype='rating'  /> : ""
				}
									...
									<TableToolbarContent>
										...
										<div style={{ width: 200 }}>
											<MultiSelect
												...
												items={columnsV2}
												initialSelectedItems={columnsV2}
								...
								<Table {...getTableProps()} useZebraStyles size='normal'>
									...
									<TableBody>
										{rows.map((row,rowIndex) => {
											...
											return <TableRow {...getRowProps({ row })} onClick={() => {this.updateSidePanelState(true,row.id);}} >
												{/*<TableSelectRow {...getSelectionProps({ row })} />*/}
												{row.cells.map((cell) => {														
														if(cell.info.header === 'name') {
															return <TableCell key={cell.id} style={{ width: '180px' }}>{cell.value}<br/>{(this.state.rows[dataRowIndex]) ? this.state.rows[dataRowIndex].url : ""}</TableCell> 
													}	
													if(cell.info.header === 'rating') {
														return <TableCell key={cell.id} >{this.viewRating(dataRowIndex,row.id)}</TableCell> 
													}
													if(cell.info.header === 'date') {
															return <TableCell key={cell.id} style={{ width: '180px' }}>{this.formateDateString(cell.value)}</TableCell> 
													}
...
export default connect(mapStateToProps,mapDispatchToProps)(FeedbackRatingView);