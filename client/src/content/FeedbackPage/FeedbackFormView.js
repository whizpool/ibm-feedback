...
class FeedbackFormView extends React.Component {
  ...
  getFeedbacks = () => {
		...
		var config = {
			...
			data:{filters:filterData,type:'feedback'}
		};		
		axios(config)
		.then(response => {
 		...
 		return (
			...
			<section className="bx--col-lg-13">			
				{ (this.state.recordID !== 0 )?		
					...
					<SidePanelViewFeedback opensidepanel ={this.state.openSidePanel} updatesidepanelstate={this.updateSidePanelState} access_token={this.props.access_token} recordid={this.state.recordID} widgettype='feedback' /> : ""
				}
											...
											return <TableRow {...getRowProps({ row })} onClick={() => {this.updateSidePanelState(true,row.id);}} >
												...
												{row.cells.map((cell) => {				
														if(cell.info.header === 'name') {
															return <TableCell key={cell.id} style={{ width: '180px' }}>{cell.value}<br/>{displayedRows[rowIndex].url}</TableCell> 
													}	
...
export default connect(mapStateToProps,mapDispatchToProps)(FeedbackFormView);